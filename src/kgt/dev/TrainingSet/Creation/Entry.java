...
public class Entry {
	...
	public void downSample(){
		...
		for(ImageMatrix imgMatrix : imgList){
			...
			System.out.println(imgMat.cols() + ", " + imgMat.rows());
		...
		for(x = 0;x < h; x++){
			for(y = 0; y < w;y++){
				...
				if(index[0] == 0 && flag){
					...
					System.out.println("H - found a pixel : " + y + "|" + x);
				}
			}
		}
		...
		for(x = h -1; x >= 0;x--){
			for(y = 0; y < w;y++){
				...
				if(index[0] == 0 && flag){
					...
					System.out.println("B-H - found a pixel : " + y + "|" + x);
				}
			}
		}
		...
		for(x = 0; x < w; x++){
			...
			for(y = 0; y < h; y++){
				...
				if(index[0] == 0 && flag){
					...
					System.out.println("L-V - found a pixel : " + x + "|" + y);
				}
			}
		}
		...
		for(x = w-1; x >= 0;x--){
			for(y = 0; y<h; y++){
				...
				if(index[0] == 0 && flag){
					...
					System.out.println("R-V - found a pixel : " + x + "|" + y);
				}
			}
		}
	}
	 ...
	 */
	public TrainingSet getTrainingSetList(){
		return ts;
	}
